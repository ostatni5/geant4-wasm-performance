...
self.onmessage = function (e) {
  if (e.data.type === "module") {
    ...
    returnResultFiles = e.data.returnResultFiles;
    runModule();
  }
};
  ...
  createModule(preModule).then((Module) => {
    ...
    const writeFile = (data) => {
      const useCustomInput =
        data && "input" in data && data.input && data.input.length > 0;
      const inputFileName = "example.in";
      const nWorkers = data.nWorkers ?? 1;

      if (useCustomInput) FS.writeFile(inputFileName, data.input);
      else
        FS.writeFile(
          inputFileName,
          FS.readFile("exampleB1.in", { encoding: "utf8" })
        );
      ...
      const dumpQuantityToFileLines = lines.filter((line) =>
        line.includes("/score/dumpQuantityToFile")
      );
      const resultFileNames = dumpQuantityToFileLines.map(
        (line) => line.split(" ")[3]
      );
      ...
      const newInputFile = inputFile.replace(
        beamOnLine,
        `/run/beamOn ${newBeamOn}`
      );
      ...
      const resultFiles = returnResultFiles
